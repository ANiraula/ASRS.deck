...
url = "https://raw.githubusercontent.com/ReasonFoundation/databaseR/master/files/ASRS_GL.csv" 
urlfile=url
data <- read_csv(url(urlfile), col_names = TRUE, na = c(""), skip_empty_rows = TRUE, col_types = NULL)

data <- data.table(data)# convert to data.table
data <-  data %>%
replace(is.na(.), 0) %>% mutate_all(as.numeric)
  ...
  #[2] Calculate Total Gain/Loss for each column (i.e. Net Change to UAL over the years)
    ...
    y = t(y)/1000000000#Saving needed columns and transposing table for graphics
...
glPlot(database = FALSE, data = NULL,
      ...
      lab5 = "<b>Permanent<br>Benefit<br>Increases<b>",
      lab6 = "<b>Legislative<br>Changes<b>",
...
### Driving Factors Jeopardizing ASRS Resiliency
...
-   **Extended Amortization Timetables and low Statutory Contributions** in the past have resulted in interest on ASRS debt exceeding the actual debt payments (negative amortization) since 2001, adding a net **$`r amo`** billion to the unfunded liabilities. 
...
-   **Permanent Benefit Increases (PBI)** given added **$`r pbi*1000`** million to unfunded liabilities since 2001.
...
assets <- filterAssets(data = assets, fy = 2001, plan = "Arizona State Retirement System")
...
```{r contributions, dpi=400, echo = FALSE, fig.cap = "Source: Pension Integrity Project analysis of ASRS actuarial valuation reports.", fig.width=8, fig.height=5, fig.align = "center", message=FALSE, warning=FALSE}
  ...
  lab9 = NULL) 
{
  ...
  if (isTRUE(net.amo)) {
    ...
    reason.data <- Data %>% transmute(year = year, 
      ...
      adec_paid = arc_adec_paid_dollar,
...
pl <- planList()
reason.data <- pullData(pl, "Arizona State Retirement System") %>% filter(year > 2000 & !duplicated(year))# (2002-2020)
...
#Pull plan-specific data from the database
pl <- planList()
reason.data <- pullData(pl, "Arizona State Retirement System") %>% filter(year > 2000 & !duplicated(year))
#View(reason.data)
#(2002-2020)
...
reason.data$contribution_rate_effective_fy <- reason.data$year+2
reason.data$fiscal_year_of_contribution <- reason.data$contribution_rate_effective_fy
#View(reason.data$fiscal_year_of_contribution)
...
reason.data$total_nc <-reason.data$gain_or_loss_due_to_normal_cost_prior_year
